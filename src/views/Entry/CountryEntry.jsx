...
if(editing === false){
    item = (
        ...
        <button onClick={toggleEditing}>Edit1</button>
        <button onClick={deleteEntryById}>Delete</button></>
    )
    if(owned === false){
        <><h3>Visited By: {author}</h3><p>Country Visited: {country}</p><p>Notes on {country}: {content}</p><p>Date Written: {new Date(date).toLocaleString('en-us')}</p><button>Copy</button></>
    }
}
return(
    <div>
